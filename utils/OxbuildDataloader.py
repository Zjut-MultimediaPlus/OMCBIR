...
class ParisDataset(Dataset):
    ...
    def __init__(self, txt, transform=None, target_transform=None, loader=default_loader):
        super(ParisDataset, self).__init__()
        # 对继承自父类的属性进行初始化
...
if __name__ == '__main__':
    ...
    train_data = ParisDataset(txt='/media/llj0571/OS/SUN397/Partitions/train.txt', transform=transformations)
    # for batch_idx, (data, target) in tqdm(enumerate(train_data), total=len(train_data)):
    #     data, target = data, target
    #     print(data)
    test_data = ParisDataset(txt='/media/llj0571/OS/SUN397/Partitions/test.txt', transform=transformations)
